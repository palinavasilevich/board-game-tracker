...
async function main() {
  ...
  // Play sessions
  console.log("Adding play sessions...");
  await prisma.playSession.create({
    data: {
      userId: alice.id,
      gameId: catan.id,
      playedAt: new Date("2026-04-10T18:00:00Z"),
      durationMinutes: 90,
      notes: "First time playing with the full group, very close game!",
      players: {
        create: [
          { name: "Alice", score: 10, isWinner: true },
          { name: "Bob", score: 9 },
          { name: "Carol", score: 7 },
        ],
      },
  ...
  await prisma.playSession.create({
    data: {
      userId: alice.id,
      gameId: pandemic.id,
      playedAt: new Date("2026-04-20T15:00:00Z"),
      durationMinutes: 60,
      notes: "Barely won on the last turn.",
      players: {
        create: [
  ...
  await prisma.playSession.create({
    data: {
      userId: bob.id,
      gameId: codenames.id,
      playedAt: new Date("2026-05-01T20:00:00Z"),
      durationMinutes: 30,
      players: {
        create: [
          { name: "Bob", isWinner: true },
          { name: "Dave", isWinner: true },
          { name: "Alice" },
          { name: "Eve" },
        ],
      },
  ...
  const [genreCount, gameCount, userCount, sessionCount, reviewCount] =
    await Promise.all([
      prisma.genre.count(),
      prisma.game.count(),
      prisma.user.count(),
  ...
  console.log(`  - ${sessionCount} play sessions`);
